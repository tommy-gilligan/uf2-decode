//! Converts binary files from Microsoft's UF2 format
...
mod test {
    ...
    fn test() {
        ...
        let (converted, family_to_target) = crate::convert_from_uf2(uf2);
        assert_eq!(
            family_to_target,
            std::collections::HashMap::from([(0xe48b_ff56, 0x1000_0000)])
        );
        assert_eq!(converted, std::fs::read("test.bin").unwrap());
    }
}
